...
insert into Control_Globals (k, v)
values ('accountLockdownFunctionEnabled', 'true');
...
create trigger if not exists Account_Modification_Update
    before update
    on Account
    for each row set new.date_modified = now(); # modification updates last modified
...
create or replace procedure Account_Lockdown_Log_Invalid_Auth(in target_account_ID integer)
    modifies sql data
procedure_label:
begin
    ...
    select ID, attempt_count, last_attempt_date
    into lockdown_ID, current_lock_counter, last_attempt
    from Account_Lockdown
    where Account_Lockdown.account_ID = target_account_ID;
    ...
    if timestampdiff(minute, last_attempt, now()) >= 45 then # previous session invalidated
        update Account_Lockdown
        set attempt_count = 0, last_attempt_date = now()
        where ID = lockdown_ID; # reset count and date
    ...
    update Account_Lockdown
    set attempt_count = current_lock_counter + 1, last_attempt_date = now()
    where ID = lockdown_ID; # save attempt count incremented
end;
...
begin
    ...
    select ID, is_locked_out, lockdown_start
    into target_account_ID, is_locked, lock_date
    from Account
...
insert into Subscription_Type (name, is_group_subscription, survey_days_lifetime,
                               survey_lifetime_extension_week_completion_count_cost, survey_count_completions_cost,
                               response_limit)
values ('Free', false, 14, 0, 1, 100),              # free sub, not corpo., 2 weeks lifetime, no price, expensive per week extension, low completion count
...
insert into Tool
    (name, input_count_cost)
values ('Response Emotional Analysis', 0.5),           # emotional analysis tool, 0.5 per 1000 inputs
...
insert into Subscription_Tool_Associations (subscription_type_ID, tool_ID, cost_ratio, data_input_count_limit)
values ((select ID from Subscription_Type where name = 'Free'),
        (select ID from Tool where name = 'Response Batch Translation'), 0, 5),               # free gets only translate

       ((select ID from Subscription_Type where name = 'Standard'),
        (select ID from Tool where name = 'Response Batch Translation'), 1, 10000),           # translate for large inputs
       ((select ID from Subscription_Type where name = 'Standard'),
        (select ID from Tool where name = 'Response File Exportation'), 1, 100),              # export up to 100 in one archive
       ((select ID from Subscription_Type where name = 'Standard'),
        (select ID from Tool where name = 'Response Compound & Custom Process'), 1, 10000),   # chain up to 10000 inputs

       ((select ID from Subscription_Type where name = 'Premium'),
        (select ID from Tool where name = 'Response Batch Translation'), 0.5,
        20000),                                                                               # translate up to 10000 at 1/2 cost
       ((select ID from Subscription_Type where name = 'Premium'),
        (select ID from Tool where name = 'Response File Exportation'), 0.9, 1000),           # export up to 1000 in one go
       ((select ID from Subscription_Type where name = 'Premium'),
        (select ID from Tool where name = 'Response Compound & Custom Process'), 0.9, 50000), # chain up to 50k
       ((select ID from Subscription_Type where name = 'Premium'),
        (select ID from Tool where name = 'Response Graph Representation'), 1,
        500),                                                                                 # represent up to 500 in a graph based on repeated data

       ((select ID from Subscription_Type where name = 'Enterprise'),
        (select ID from Tool where name = 'Response Batch Translation'), 0.8, 50000),
       ((select ID from Subscription_Type where name = 'Enterprise'),
        (select ID from Tool where name = 'Response File Exportation'), 0.9, 500),
       ((select ID from Subscription_Type where name = 'Enterprise'),
        (select ID from Tool where name = 'Response Compound & Custom Process'), 0.6, 10000),
       ((select ID from Subscription_Type where name = 'Enterprise'),
        (select ID from Tool where name = 'Response Emotional Analysis'), 1,
        20000),                                                                               # analyse up to 20k inputs for emotional text ratios
       ((select ID from Subscription_Type where name = 'Enterprise'),
        (select ID from Tool where name = 'Response Graph Representation'), 1, 20000),

       ((select ID from Subscription_Type where name = 'Professional'),
        (select ID from Tool where name = 'Response Batch Translation'), 0.5, 100000),
       ((select ID from Subscription_Type where name = 'Professional'),
        (select ID from Tool where name = 'Response File Exportation'), 0.8, 5000),
       ((select ID from Subscription_Type where name = 'Professional'),
        (select ID from Tool where name = 'Response Compound & Custom Process'), 0.3, 100000),
       ((select ID from Subscription_Type where name = 'Professional'),
        (select ID from Tool where name = 'Response Emotional Analysis'), 0.7, 100000),
       ((select ID from Subscription_Type where name = 'Professional'),
        (select ID from Tool where name = 'Response Graph Representation'), 0.6, 100000),
       ((select ID from Subscription_Type where name = 'Professional'),
        (select ID from Tool where name = 'Response Data and Prediction Analysis'), 1, 5000), # predict up to 5k inputs
       ((select ID from Subscription_Type where name = 'Professional'),
        (select ID from Tool where name = 'Response Time and Trace Data'), 1, 5000);
# traceback location & time for inputs for up to 5k
...
insert into Survey_Question_Answer_Type (type)
values ('text'),  # raspuns in text
       ('yesno'), # da / nu
...
insert into Active_Subscription (subscription_type_ID)
values (1);
insert into Survey (name, subscription_used)
values ('test', 1);
insert into Survey_Question (survey_ID, question_text)
values (1, 'test4');

create trigger if not exists Survey_Question_Number_Auto_Increment
    before insert
    on Survey_Question
    for each row
begin
    ...
    if rownum != 0 then
        set new.question_number =
                (select max(question_number) + 1 from Survey_Question where survey_ID = new.survey_ID);
...
create table if not exists Survey_Question_Answer_Type_Association
( # o intrebarea poate avea un rasp atat yes/no + text, sau yes, if yes -> rate -> text, sau doar text / rate / yesno
    ID                             integer primary key auto_increment not null,
